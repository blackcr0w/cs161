...
# fileno is used to decid the user number
def socket_to_username(s):
    return "user %d" % s.fileno()

# Send msg to all connected clients.
def broadcast(msg):
    for s in tuple(CLIENT_SOCKETS):  # convert collection: CLIENT_SOCKET into tuple
        try:
            s.sendall(msg)
        except socket.error:
...
try:
    ...
    listen_port = int(listen_port)  # change string into int
except ValueError:
    usage(sys.stderr)
    sys.exit(1)
