...
EC_KEY* gen_time_key(EC_KEY *key)
{
  unsigned char buf2[32];
  int i;
  srand(time(1443700800));
  for (i = 0; i < 32; i++) {
       key[i] = rand() & 0xff;
  }
...
int main(int argc, char *argv[])
{
	...
	// rc = EC_POINT_get_affine_coordinates_GFp(ec_group, pubkey, x, y, NULL);
	// printf("%s\n", "here 2");
	// if (rc != 1)
	// 	goto err;
	...
	/* Build on top of the head of the main chain. */
      ...
      transaction_set_dest_privkey(&(newblock->reward_tx), key);
       printf("%s\n", "here 2.2");
      ...
      transaction_set_prev_transaction(&newblock->normal_tx,
       ...
       printf("%s\n", "here 2.2");
      /* Send it to us. */
      transaction_set_dest_privkey(&newblock->normal_tx, key);
       printf("%s\n", "here 2.3");
	...
	while (1) {
		 EC_KEY *key = gen_time_key();
		 if (comparekey(key, old_owner_key))
		 	break;
	}
