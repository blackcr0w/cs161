...
static void generate_prime(mpz_t p, unsigned int numbits)
{
	...
	do {
		...
		n = fread(buf, len, 1, urandom);
		if (n != 1)
			abort();
		/* Set the two most significant bits so that this number is
		 * within half a bit of numbits. When two such numbers are
		 * multiplied, their product will be within one bit of the
		 * target number of bits. */
		buf[0] = buf[0] | 0xc0;
		mpz_import(p, len, 1, 1, 0, 0, buf);
...
void rsa_genkey(struct rsa_key *key, unsigned int numbits)
{
	...
	mpz_t p, q, p_minus_1, q_minus_1, phi;
	...
	/* If e is not invertible, something has gone really wrong. */
